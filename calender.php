 ...
 <!--+++++++++Form Start HERE for ADd_ canelder data++++++++++++-->
  ...
  <input type="text" id="placeid_cal" value="<?php echo $pid; ?>" hidden>
...
<input type="text" id="ppm" required placeholder="Price Per Week" class="form-control">
 ...
 <!--  <div class="col-md-12 mg-top10">
  ...
  <div class="col-md-6"><button class="btn-3 btn-custom2 myset" type="button" name="values">Set</button></div></div></form>
...
$('.avail1').click(function(){
  ...
  var pph = $('#pph').val();
   ...
   console.log('label='+label+'&datefirst='+date1+'&datelast='+date2+'&status='+status+'&pph='+pph+'&ppn='+ppw+'&ppw='+ppm);
     $.ajax({
           url: 'forms.php?label='+label+'&datefirst='+date1+'&datelast='+date2+'&status='+status+'&pph='+pph+'&ppn='+ppw+'&ppw='+ppm+'&placeid_cal='+place_ids,
           contentType: false,
           cache: false,
           processData:false,
           ...
           {    
            console.log(data);
             ...
             { 
              ...
              $('.start').addClass('last');
              window.location.href="edit-place.php?placeid="+place_ids+"&unique=100"
             }
             else
             { 
              $('#selecteddates').load(window.location + ' #selecteddates');
