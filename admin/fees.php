 ...
 <?php include("header.php");?>
        ...
        <div class="row">
          <?php  if(isset($_REQUEST['adel']))
                      {
                         $id=$_REQUEST["adel"];
            $query=mysqli_query($connect,'DELETE FROM `fees` WHERE feeid="'.$id.'"');
                          if($query>0){
        ...
        <a href="addFees.php" class="btn btn-danger btn-xs" type="button">Add Fees</a>
        <div class="adv-table">
        ...
        <?php
        $i=1;
        $query=mysqli_query($connect,'SELECT * FROM `fees` order by feeid desc');
        ...
        <tr class="gradeX">
            ...
            <td style="width:20%" class="center hidden-phone">                
                <a href="editfees.php?id=<?php echo $row["feeid"];?>" class="btn btn-warning btn-xs" type="button">Edit</a>
                <a href="?adel=<?php echo $row["feeid"];?>" class="btn btn-danger btn-xs" type="button">Delete</a>
            </td>
        </tr>
       <?php
       }?>
