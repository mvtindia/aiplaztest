 ...
 <?php include("header.php");?>
        ...
        <div class="row">
          <?php  if(isset($_REQUEST['adel']))
                      {
                         $id=$_REQUEST["adel"];
            $query=mysqli_query($connect,'DELETE FROM `safety` WHERE sid="'.$id.'"');
                          if($query>0){
        ...
        <a href="addsafety.php" class="btn btn-danger btn-xs" type="button">Add Safety</a>
        <div class="adv-table">
        ...
        <?php
        $i=1;
        $query=mysqli_query($connect,'SELECT * FROM `safety` order by sid desc');
        ...
        <tr class="gradeX">
            ...
            <td style="width:10%"><?php echo $row['sname'];?></td>
            <td style="width:20%" class="center hidden-phone">                
                <a href="editsafety.php?id=<?php echo $row["sid"];?>" class="btn btn-warning btn-xs" type="button">Edit</a>
